...
**Key global found:** `sub_8229CA28` (recomp.13:8176) iterates a **global array at `0x82CE3A38`**
(`lis -32114; addi 14904`), searching entries (stride ~ the 40-byte struct; key at `[entry-60]`,
a 64-bit value = XUID?) for one matching `r3`, and returns it. So the load writes the profile into
a **global per-user save-slot** at `0x82CE3A38`. Persistent data IS loaded into a global.

## The open question (next session starts here)
The load populates the `0x82CE3A38` save-slot, yet the level-select shows level 1. So either:
1. **The level-select unlock reads a DIFFERENT global** (an in-memory "campaign progress" struct),
   and the copy `save-slot(0x82CE3A38) → campaign-progress` does NOT run on CAMPAIGN/LOCAL-GAME
...
- **Instrument the save-slot global:** log reads/writes of `0x82CE3A38` region (or the
...
- The runtime has `[PROF-*]` diagnostics (in `patches/rexglue-sdk-current-full.patch`); add
  `0x82CE3A38`-watch logging similarly.
...
the loaded `0x82CE3A38` slot to the campaign-progress state. That's guest-side: a config function
